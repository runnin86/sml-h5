  ...
  <div @click.stop="" @touchstart.stop="" class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
    <div class="calendar-tools">
      ...
      <div class="text center">
        <input type="number" v-model="year" value="{{year}}" @change="render(year,month)" min="1970" max="2100" maxlength="4" number>/ {{month+1}}</div></div>
    <table cellpadding="5">
      <thead>
        <tr>
          <td v-for="week in weeks" class="week">{{week}}</td></tr>
      </thead>
      <tr v-for="(k1,day) in days">
        <td v-for="(k2,child) in day" :class="{'today':child.today,'disabled':child.disabled}" @click="select(k1,k2,$event)" @touchstart="select(k1,k2,$event)">{{child.day}}
    ...
    <div class="calendar-time" v-show="type=='datetime'||type=='time'">
      <div class="timer">
        ...
        <input type="number" v-model="second" value="{{second}}" min="0" max="59" maxlength="2" number>秒</div></div>
    <div class="calendar-button" v-show="type=='datetime'||type=='time'||range">
      <button @click="ok">确定</button>
      <button @click="cancel" class="cancel">取消</button></div>
  </div>
</template>
...
module.exports = {
  ...
  created: function () {
    ...
    if (that.value !== '') {
      ...
      if (that.type === 'date') {
        let split = that.value.split(that.sep)
        that.year = parseInt(split[0], 0)
        that.month = parseInt(split[1], 0) - 1
        that.day = parseInt(split[2], 0)
      }
      else if (that.type === 'datetime') {
        let split = that.value.split(' ')
        let splitDate = split[0].split(that.sep)
        that.year = parseInt(splitDate[0], 0)
        that.month = parseInt(splitDate[1], 0) - 1
        that.day = parseInt(splitDate[2], 0)
        if (split.length > 1) {
      ...
      if (that.range) {
        let split = that.value.split(' ~ ')
        if (split.length > 1) {
          let beginSplit = split[0].split(that.sep)
          let endSplit = split[1].split(that.sep)
          that.rangeBegin = [parseInt(beginSplit[0], 0), parseInt(beginSplit[1] - 1, 0), parseInt(beginSplit[2], 0)]
          that.rangeEnd = [parseInt(endSplit[0], 0), parseInt(endSplit[1] - 1, 0), parseInt(endSplit[2], 0)]
        }
      }
    }
    else {
  ...
  methods: {
    ...
    render: function (y, m) {
      ...
      let seletSplit = that.value.split(' ')[0].split(that.sep)
      let i = 0
      let line = 0
      let temp = []
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (that.range) {
          ...
          if (that.rangeBegin.length > 0) {
            let beginTime = Number(new Date(that.rangeBegin[0], that.rangeBegin[1], that.rangeBegin[2]))
            let endTime = Number(new Date(that.rangeEnd[0], that.rangeEnd[1], that.rangeEnd[2]))
            let thisTime = Number(new Date(that.year, that.month, i))
          ...
          if (parseInt(seletSplit[0], 0) === that.year && parseInt(seletSplit[1], 0) - 1 === that.month && parseInt(seletSplit[2], 0) === i && (that.begin === undefined || that.end === undefined)) {
            temp[line].push({
              day: i,
              today: true
            })
            ...
            if (that.begin !== undefined) {
              let beginSplit = that.begin.split(that.sep)
              let beginTime = Number(new Date(parseInt(beginSplit[0], 0), parseInt(beginSplit[1], 0) - 1, parseInt(beginSplit[2], 0)))
              let thisTime = Number(new Date(that.year, that.month, i))
              if (beginTime > thisTime) options.disabled = true
            }
            if (that.end !== undefined) {
              let endSplit = that.end.split(that.sep)
              let endTime = Number(new Date(parseInt(endSplit[0], 0), parseInt(endSplit[1], 0) - 1, parseInt(endSplit[2], 0)))
              let thisTime = Number(new Date(that.year, that.month, i))
